...
def calculate_logistic_r(df):
    min_value = df['close'].min()
    max_value = df['close'].max()
    
    # Use the formula r = x_{n+1} / (x_n * (1 - x_n))
    # r = max_value / (min_value * (1 - min_value))
    r = round( min_value / max_value ,1 )* 6
    
    return r

def calculate_initial_x0(df):
    # Calculate the average of df['close']
    average_value =  df['close'].min() / df['close'].mean() 
    
    return round(average_value, 1)

def plot_data(ticker, prediction_interval, start_date, end_date):
    try:
        ...
        # r = 3.7
        # x0 = 0.5
        r = calculate_logistic_r(df)
        x0 = calculate_initial_x0(df)

        print(f"calculate_logistic_r, {r}!")
...
def plot_data_nn(ticker, prediction_interval, start_date, end_date):
    try:
        df = fetch_data(ticker, start_date, end_date)
...
def on_button_click():
    ...
    plot_data(ticker, prediction_interval, start_date, end_date)
...
def on_button_nn_click():
    ...
    start_date = start_date_entry.get()
    end_date = end_date_entry.get()
    plot_data_nn(ticker, prediction_interval, start_date, end_date)
...
start_date_entry.insert(0, "2022-01-01")
