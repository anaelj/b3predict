...
def train_neural_network(df, time_steps=10, epochs=50):
...
def plot_data_nn(ticker, prediction_interval):
    try:
        ...
        plt.plot(df['time'][:len(nn_predictions)], nn_predictions, label='Neural Network Predictions', linestyle='-', marker='s')
